...
export default function Navbar() {
  ...
  return (
    ...
    className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled && !menuOpen
        ? 'bg-background-50/20 backdrop-blur-md py-3'
        : scrolled
          ? 'bg-transparent py-3'
          : 'bg-transparent py-5'
    }`}
  >
      <div className="w-full px-6 md:px-14 flex items-center justify-between">
        ...
        <button
          ...
          className="md:hidden w-10 h-10 flex items-center justify-center cursor-pointer z-[60] relative"
          aria-label="メニューを開く"
        >
        ...
        <div
          ...
          className={`fixed inset-0 bg-black/40 z-[55] transition-opacity duration-500 md:hidden ${menuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
            }`}
        ></div>

        {/* Mobile menu panel */}
        <div
          className={`fixed top-0 right-0 h-full w-72 z-[58] transition-transform duration-500  ease-out md:hidden flex flex-col ${menuOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
